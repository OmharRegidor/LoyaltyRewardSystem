-- Fix: process_staff_sale fails because p_staff_id is TEXT but
-- sales.staff_id, scan_logs.staff_id, and staff.id are all UUID.
-- PL/pgSQL does not implicitly cast TEXT → UUID in INSERT/UPDATE statements.
-- Solution: add explicit p_staff_id::UUID casts where needed.
...
BEGIN
  ...
  -- Calculate points earned (based on total after discount but before exchange)
  ...
  INSERT INTO sales (
    ...
    p_business_id, p_customer_id, v_staff_uuid, v_sale_number,
  ...
  VALUES (p_business_id, v_staff_uuid, p_customer_id, v_points_earned, v_total_centavos);
