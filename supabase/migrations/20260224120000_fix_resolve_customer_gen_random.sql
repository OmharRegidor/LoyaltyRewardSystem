-- Fix: resolve_customer_for_business used gen_random_bytes() from pgcrypto
-- which is in the 'extensions' schema, but the function's search_path is 'public'.
-- Replace with gen_random_uuid() which is always available.
...
BEGIN
  ...
  -- Use gen_random_uuid() instead of gen_random_bytes() to avoid pgcrypto schema dependency
