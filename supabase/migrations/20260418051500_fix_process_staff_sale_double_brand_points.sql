-- Per-brand customer_businesses.points was being doubled because:
--   (a) the RPC did UPDATE customer_businesses SET points = points + earned
--   (b) the AFTER INSERT trigger trg_auto_link_customer_business on transactions
--       also upserted customer_businesses adjusting points
-- Remove (a) — the trigger is the single source of truth for per-brand points
-- (it also handles auto-linking when the customer first transacts at the brand).
-- customers.total_points is still updated by the RPC only.
...
BEGIN
  ...
  -- Earn: update global total_points only; trigger on transactions handles per-brand.
  ...
  -- Redeem: update global total_points only; trigger on transactions subtracts per-brand.
