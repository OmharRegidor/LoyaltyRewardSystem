-- Use item names in the transaction description (e.g., "Americano x2, Coffee Beans")
-- so the mobile wallet shows the actual item instead of a generic "Purchase" label.
-- Falls back to "POS Sale #<number>" when no items were passed.
...
BEGIN
  ...
  -- Compose transaction description from item names: "Americano x2, Coffee Beans"
  SELECT string_agg(
