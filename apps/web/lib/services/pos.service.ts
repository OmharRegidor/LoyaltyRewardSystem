...
export async function updateProduct(
  ...
  if (input.price !== undefined) updateData.price_centavos = Math.round(input.price * 100);
...
function calculatePointsEarned(totalCentavos: number, business: Business): number {
...
export async function getSaleById(id: string): Promise<SaleWithItems | null> {
  ...
  if (saleError || !sale) {
    if (saleError?.code === '42P01' || saleError?.message?.includes('does not exist')) {
      return null;
    }
...
export async function getDailySummary(
  ...
  if (saleIds.length > 0) {
    ...
    total_items_sold = (items || []).reduce((sum: number, item: { quantity: number }) => sum + item.quantity, 0);
  }
...
export async function getSalesAnalytics(
  ...
  const productAggregation: Record<string, { name: string; quantity: number; revenue: number }> = {};
  ...
  if (saleIds.length > 0) {
    ...
    if (items) {
      items.forEach((item: { product_id: string | null; name: string; quantity: number; total_centavos: number }) => {
        totalItemsSold += item.quantity;

        const key = item.product_id || item.name;
        if (!productAggregation[key]) {
          productAggregation[key] = { name: item.name, quantity: 0, revenue: 0 };
        }
