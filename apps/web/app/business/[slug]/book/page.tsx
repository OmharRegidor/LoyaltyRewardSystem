// apps/web/app/business/[slug]/book/page.tsx
...
export default function PublicBookingPage({ params }: BookingPageProps) {
  ...
  // Fetch business and services on mount
  useEffect(() => {
  ...
  useEffect(() => {
    async function fetchTimeSlots() {
      if (!business || !formData.booking_date || !formData.service_id) {
        setTimeSlots([]);
        return;
      }
    ...
    fetchTimeSlots();
  }, [business, formData.booking_date, formData.service_id, selectedService, supabase]);
  ...
  async function handleSubmit() {
    ...
    try {
      const endTime = minutesToTime(
        timeToMinutes(formData.start_time) + selectedService.duration_minutes
      );
              ...
              {services.map((service) => (
                <button
                  ...
                  className={`w-full rounded-lg border p-4 text-left transition-colors hover:bg-accent ${
                    formData.service_id === service.id ? 'border-primary bg-accent' : ''
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <p className="font-semibold">{formatPrice(service.price_centavos)}</p>
                  </div>
                </button>
              ))}
            </CardContent>
